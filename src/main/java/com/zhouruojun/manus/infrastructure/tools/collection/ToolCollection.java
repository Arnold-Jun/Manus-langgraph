package com.zhouruojun.manus.infrastructure.tools.collection;
...
public class ToolCollection {
    ...
    public ToolCollection(boolean internalToolExecutionEnabled, boolean autoScanTools) {
        this(internalToolExecutionEnabled, autoScanTools, (String) null);
    }
     ...
     * @param internalToolExecutionEnabled 是否启用内部工具执行
     * @param autoScanTools 是否自动扫描并注册工具
     * @param toolType 工具类型，用于扫描特定类型的工具目录
     */
    public ToolCollection(boolean internalToolExecutionEnabled, boolean autoScanTools, String toolType) {
    ...
    public ToolCollection(boolean internalToolExecutionEnabled, boolean autoScanTools, ToolCallback... callbacks) {
        this(internalToolExecutionEnabled, autoScanTools, null, callbacks);
    }

    /**
     * 最完整的构造函数，包含所有可配置选项和工具类型
     * @param internalToolExecutionEnabled 是否启用内部工具执行
     * @param autoScanTools 是否自动扫描并注册工具
     * @param toolType 工具类型，用于扫描特定类型的工具目录
     * @param callbacks 初始工具回调列表
     */
    public ToolCollection(boolean internalToolExecutionEnabled, boolean autoScanTools, String toolType, ToolCallback... callbacks) {
        this.internalToolExecutionEnabled = internalToolExecutionEnabled;
        this.autoScanTools = autoScanTools;
        this.toolType = toolType;
    ...
    private void scanAndRegisterTools() {
        try {
            ...
            // 1. 始终扫描通用工具目录
            packageNames.add("com.zhouruojun.manus.infrastructure.tools.imp.common");
            
            // 2. 如果指定了工具类型，也扫描对应类型的工具目录
            if (toolType != null && !toolType.trim().isEmpty()) {
                packageNames.add("com.zhouruojun.manus.infrastructure.tools.imp." + toolType);
            }
            ...
            for (String packageName : packageNames) {
                log.info("正在扫描工具包: " + packageName);
                
                // 获取class文件所在目录
                String path = packageName.replace('.', '/');
                Enumeration<URL> resources = classLoader.getResources(path);

                int packageToolCount = 0;
                            ...
                            for (File file : files) {
                                ...
                                try {
                                    ...
                                    if (Function.class.isAssignableFrom(clazz) &&
                                        ...
                                        packageToolCount += registerMethodTool(clazz);
                                    }
                                } catch (Exception e) {
                ...
                if (packageToolCount > 0) {
                    log.info("从包 " + packageName + " 中成功注册了 " + packageToolCount + " 个工具");
                } else {
                    log.info("包 " + packageName + " 中没有找到可注册的工具");
                }
                
                totalToolCount += packageToolCount;
            }

            log.info("工具扫描完成，总共注册了 " + totalToolCount + " 个工具 (工具类型: " + (toolType != null ? toolType : "通用") + ")");
