...
public class WorkflowEngine {
     ...
     * 继续执行工作流（用于人工输入后继续）
     */
    public CompletableFuture<String> continueWorkflow(String userInput, String sessionId) {
        return CompletableFuture.supplyAsync(() -> {
            try {
                RunnableConfig config = RunnableConfig.builder()
                        .threadId(sessionId)
                ...
                // 获取当前状态并更新用户输入
                var currentState = compiledGraph.getState(config);
                ...
                // 更新状态
                RunnableConfig newConfig = compiledGraph.updateState(config, updatedData);

                // 继续执行
                AsyncGenerator<NodeOutput<AgentMessageState>> stream =
                        compiledGraph.stream(null, newConfig);
                ...
                log.error("Error continuing workflow", e);
                return "继续执行过程中发生错误: " + e.getMessage();
            }
        });
    }
